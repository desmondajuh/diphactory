// features/gallery/components/gallery-delete-modal.tsx
"use client";
...
export function GalleryDeleteModal({ image, onCancel, onConfirm }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#0e0e0e] p-6 space-y-4">
        <div>
          <p className="text-base font-semibold text-white">Delete image?</p>
          <p className="mt-1 text-sm text-white/40">
            <span className="text-white/60">{image.filename}</span> will be
